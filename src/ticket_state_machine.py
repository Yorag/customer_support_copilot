...
_ALLOWED_PROCESSING_STATUS_TRANSITIONS: dict[
    ...
    TicketProcessingStatus.LEASED: (TicketProcessingStatus.RUNNING,),
...
class TicketStateService:
    ...
    def claim_ticket(
        ...
        *,
        ...
        if TicketProcessingStatus(ticket.processing_status) not in {
            TicketProcessingStatus.QUEUED,
            TicketProcessingStatus.ERROR,
        }:
            raise InvalidStateTransitionError(
                entity="ticket.processing_status",
                current_status=ticket.processing_status,
        ...
        if not self._lease_is_available(ticket, current_time):
    ...
    def start_run(
        ...
        *,
        ...
        self._assert_valid_lease(ticket, worker_id=worker_id, now=now)
        return self.update_ticket_statuses(
            ticket.ticket_id,
            update=TicketStatusUpdate(
    ...
    def renew_lease(
        ...
        *,
        ...
        current_time = ensure_timezone_aware(now or utc_now())
        self._assert_valid_lease(ticket, worker_id=worker_id, now=current_time)
        return self.update_ticket_statuses(
            ticket.ticket_id,
            update=TicketStatusUpdate(
    ...
    def mark_waiting_external(
        ...
        *,
        ...
        if normalized_business_status not in {
            TicketBusinessStatus.AWAITING_CUSTOMER_INPUT,
            TicketBusinessStatus.AWAITING_HUMAN_REVIEW,
            TicketBusinessStatus.ESCALATED,
        }:
            raise InvalidStateTransitionError(
                entity="ticket.business_status",
                current_status=ticket.business_status,
                target_status=normalized_business_status.value,
                reason=(
    ...
    def requeue_failed_ticket(
        ...
        *,
        ...
        if TicketBusinessStatus(ticket.business_status) is not TicketBusinessStatus.FAILED:
            raise InvalidStateTransitionError(
                entity="ticket.business_status",
                current_status=ticket.business_status,
        ...
        if TicketProcessingStatus(ticket.processing_status) is not TicketProcessingStatus.ERROR:
            raise InvalidStateTransitionError(
                entity="ticket.processing_status",
                current_status=ticket.processing_status,
    ...
    def reclaim_expired_lease(
        ...
        *,
        ...
        current_time = ensure_timezone_aware(now or utc_now())
        if ticket.lease_expires_at is None or ticket.lease_expires_at > current_time:
            raise LeaseConflictError(
                ticket_id=ticket.ticket_id,
                lease_owner=ticket.lease_owner,
    ...
    def apply_manual_review_action(
        ...
        *,
        ...
        if normalized_action in {
            ...
            updated_ticket = self.update_ticket_statuses(
                ticket.ticket_id,
                update=TicketStatusUpdate(
            ...
            updated_ticket = self.update_ticket_statuses(
                ...
                update=TicketStatusUpdate(
                    ...
                    processing_status=TicketProcessingStatus.WAITING_EXTERNAL,
                    fields={"risk_reasons": ticket.risk_reasons, "lease_owner": None},
                    clear_lease=True,
                ),
            )
    ...
    def _routing_fields_updated(self, fields: Mapping[str, Any]) -> bool:
        return bool(
            {"source_channel", "primary_route", "secondary_routes", "tags", "multi_intent"}
            .intersection(fields.keys())
        )

    def _lease_is_available(self, ticket: Ticket, now: datetime) -> bool:
        if ticket.lease_expires_at is None:
            return True
        return ticket.lease_expires_at <= now
