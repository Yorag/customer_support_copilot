...
`evals/samples/customer_support_eval_zh.jsonl`（53 条中文样本）

### 2.2 场景覆盖

| 场景类型 | 样本数 | 样本 ID | 预期行为 |
|---|---|---|---|
| `knowledge_supported` | 8 | `zh_knowledge_001`~`008` | 知识库有答案 → 草稿回复 |
| `knowledge_gap` | 7 | `zh_kb_gap_001`~`007` | 知识库无答案 → 保守草稿，不虚构 |
| `technical_issue_clarification` | 8 | `zh_tech_clarify_001`~`008` | 信息不充分 → 索要诊断信息 |
| `technical_issue_detailed` | 8 | `zh_tech_detail_001`~`008` | 信息充分 → 直接排查建议 |
| `commercial_policy_high_risk` | 7 | `zh_policy_001`~`002`、`004`~`008` | 退款/SLA/合同争议 → 必须升级人工 |
| `commercial_policy_standard` | 4 | `zh_policy_003`、`009`~`011` | 常规取消订阅/发票说明 → 可自动草稿 |
| `feedback_intake` | 4 | `zh_feedback_001`~`004` | 功能建议/体验反馈 → 确认收到 |
| `unrelated` | 3 | `zh_unrelated_001`~`003` | 商务外联/招聘服务 → 识别并关闭 |
| `multi_intent` | 4 | `zh_multi_001`~`004` | 混合意图 → 优先高风险路由 |
