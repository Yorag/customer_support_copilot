...
1. 通过 `RAG MCP` 插件接入外部知识库。
2. 增加更多业务规则，例如优先级、SLA、客户等级。
3. 支持更多入口渠道，例如 Web 表单或工单系统。
4. 建立轻量控制台或 trace 查询页面。
5. 支持邮件图片附件的最小多模态处理，但不改变文本优先的意图识别机制。
...
这里的“理想路径”指的是：  
某类邮件在正常情况下应经过哪些节点、调用哪些能力、在什么条件下转人工。
...
1. 能明确区分“系统快不快”和“系统答得对不对”。
2. 能回答“这次改动是让系统变好了还是变坏了”。
3. 能定位问题是出在结果质量，还是出在 Agent 轨迹。
...
4. 人工审核动作和自动流程同时更新同一工单状态。
5. 多个 worker 同时争抢待处理工单。
...
“我从一个基于 LangGraph 的邮件自动化仓库出发，把它重构成了一个以邮件为入口的多 Agent 客服协同系统。系统先把邮件工单化，再由 Triage Agent 做意图识别、优先级判断和升级判断；如果需要知识或规则判断，就交给 Knowledge & Policy Agent 访问本地 RAG 和策略约束；之后由 Drafting Agent 生成回复，由 QA & Handoff Agent 做质量审查并在必要时输出人工升级说明。为了让系统不是一次性脚本，我又补了短期记忆、长期记忆、最小工具层、工单状态、并发控制和两层观测能力。当前 RAG 我保留了原项目的本地实现，没有优先重写，只把未来外部知识接入设计成 `RAG MCP` 插件接口。这样项目重点就集中在 LangGraph 状态编排、路由、记忆、工具边界和评估上，更适合作为面试项目。”
