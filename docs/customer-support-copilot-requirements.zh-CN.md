...
5. 引入短期记忆与长期记忆。
6. V1 中由 `QA & Handoff Agent` 输出人工升级结论与交接说明，但不实现正式人工审核状态机。
...
1. 邮件接入层，对应渠道接入层。
2. 意图识别与路由层、多 Agent 协作层、草稿生成与改写层，以及 V2 的人工审核与升级层，主要落在 LangGraph 编排层加 Agent 能力层。
...
本节属于 V2 版本内容，用于在 V1 自动化服务版稳定后，建立真正的人在回路中，而不是只生成草稿就结束。
...
如果 V2 再引入正式人工审核状态机，还需要额外处理人工审核动作和自动流程同时更新同一工单状态的问题。
...
“我从一个基于 LangGraph 的邮件自动化仓库出发，把它重构成了一个以邮件为入口的多 Agent 客服协同系统。系统先把邮件工单化，再由 Triage Agent 做意图识别、优先级判断和升级判断；如果需要知识或规则判断，就交给 Knowledge & Policy Agent 访问本地 RAG 和策略约束；之后由 Drafting Agent 生成回复，由 QA & Handoff Agent 做质量审查，并在必要时输出人工升级说明。为了让系统不是一次性脚本，我在 V1 里补了短期记忆、长期记忆、最小工具层、工单状态、并发控制以及基于 trace 的评估观测能力，重点观察延迟、资源、响应质量和执行轨迹。正式人工审核闭环放在 V2，再继续补人机协作状态机和人工回写。当前 RAG 我保留了原项目的本地实现，没有优先重写，只把未来外部知识接入设计成 `RAG MCP` 插件接口。这样项目重点就集中在 LangGraph 状态编排、路由、记忆、工具边界和评估上，更适合作为面试项目。”
