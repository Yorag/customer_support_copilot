...
2. 维护处理状态。
3. 记录 V1 的升级结论，并为 V2 的人工审核结果预留状态位。
4. 提供可恢复性和幂等控制。
...
如果 V2 再引入正式人工审核状态机，还要额外处理人工审核和自动流程同时更新同一工单的问题。
...
1. 业务状态，例如 `triaged`、`draft_created`、`escalated`。
...
4. 输出重写建议或通过结论。
5. 在需要时决定是否人工接管。
6. V1 只输出升级原因和人工接手说明，V2 再补正式人工审核闭环。
...
本节属于 V2 版本内容。当前项目只生成草稿，V1 只输出升级结论和 handoff 说明，还没有正式的人审状态机。
...
| 审核 | Proofreader 二分类 | V1: QA + 升级说明；V2: 人工审核 + 升级 |
